...
@bot.on(admin_cmd(pattern="encode ?(.*)"))
async def encod(e):
    ...
    if not (match or e.is_reply):
        return await e.edit("`Give me Something to Encode..`")
    byt = match.encode("ascii")
    et = base64.b64encode(byt)
    atc = et.decode("ascii")
    await e.reply(f"**=>> Encoded Text :** `{match}`\n\n**=>> OUTPUT :**\n`{atc}`")
...
async def encod(e):
    ...
    if not match and e.is_reply:
        ...
        if gt.text:
            match = gt.text
    if not (match or e.is_reply):
        return await e.edit("`Give me Something to Decode..`")
    byt = match.encode("ascii")
    try:
        et = base64.b64decode(byt)
        atc = et.decode("ascii")
        await e.reply(f"**=>> Decoded Text :** `{match}`\n\n**=>> OUTPUT :**\n`{atc}`")
    except Exception as p:
        await e.reply("**ERROR :** " + str(p))
