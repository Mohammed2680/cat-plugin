...
@borg.on(admin_cmd(pattern="dlink ?(.*)",incoming=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.edit("```Please restart me (@uploadbot) u Nigga```")
            return
        await event.delete()
...
@borg.on(admin_cmd(pattern="cvid ?(.*)",incoming=True))
async def _(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
        ...
        return
    reply_message = await event.get_reply_message()
    chat = "@CompressorRobot"
    reply_message.sender
    await event.edit("**Making Downloading url...**")
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=1662138215)
            )
            await event.client.forward_messages(chat, reply_message)
            response = await response
        except YouBlockedUserError:
            await event.edit("```Please restart me (@CompressorRobot) u Nigga```")
            return
        await event.delete()
        await event.client.send_message(
            event.chat_id, response.message, reply_to=reply_message
